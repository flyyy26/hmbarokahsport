...
class CustomerVoucherController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // 🔥 TAB AKTIF (semua | riwayat)
        $tab = $request->get('tab', 'semua');
        if (!in_array($tab, ['semua', 'riwayat'])) {
            $tab = 'semua';
        }
        ...
        if ($customerId) {
            if ($tab === 'riwayat') {
                // Tampilkan HANYA voucher yang sudah pernah dipakai user
                $query->whereExists(function($sub) use ($customerId) {
                    $sub->select('id')
                        ->from('voucher_usages')
                ...
                // Tab "semua" → HANYA voucher yang BELUM dipakai user
                $query->whereNotExists(function($sub) use ($customerId) {
                    $sub->select('id')
                        ->from('voucher_usages')
                        ->whereColumn('voucher_usages.voucher_id', 'vouchers.id')
                        ->where('voucher_usages.user_id', $customerId);
            ...
            // User tidak login → hanya tab "semua" (tanpa riwayat)
            if ($tab === 'riwayat') {
                $tab = 'semua';
            }
        }
        ...
        if ($customerId) {
            $baseCountQuery = Voucher::where('is_active', true)
                ->where('is_public', true)
                ->where('start_date', '<=', now())
                ->where('end_date', '>=', now())
                ->where(function($q) {
            ...
            $historyCount = (clone $baseCountQuery)
            ...
            $allCount = Voucher::where('is_active', true)
                ->where('is_public', true)
                ->where('start_date', '<=', now())
                ->where('end_date', '>=', now())
                ->where(function($q) {
