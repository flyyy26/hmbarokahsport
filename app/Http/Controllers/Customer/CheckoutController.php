    ...
    public function index()
{
    ...
    $guestData = session()->get('guest_checkout_data', []);
    ...
    public function getShippingCost(Request $request)
    {
        try {
            ...
            if (empty($originPostalCode)) {
                ...
                $originPostalCode = (string) ($setting->postal_code ?? config('services.biteship.origin_postal_code', '46191'));
            }
    ...
    public function updateShipping(Request $request)
{
    ...
    public function process(Request $request)
    {
        ...
        // 🔥 RESTORE CART DARI SESSION GUEST (jika hilang akibat login)
        if (empty($cart) && session()->has('guest_checkout_cart')) {
            $cart = session()->get('guest_checkout_cart', []);
            session()->put('cart', $cart);
        }
        ...
        // 🔥 JIKA USER LOGIN DAN PILIH ALAMAT DARI DATABASE
        if ($request->filled('address_id') && Auth::guard('customer')->check()) {
            $selectedAddress = UserAddress::where('user_id', Auth::guard('customer')->id())
            ...
            $request->merge([
                ...
                'shipping_subdistrict' => $selectedAddress->subdistrict,
                'shipping_postal_code' => $selectedAddress->postal_code ?? '0',
            ]);
        }
        ...
        try {
            ...
            if (Auth::guard('customer')->check()) {
                ...
                // 🔥 SIMPAN ALAMAT JIKA USER TIDAK MENGIRIM address_id
                // (misalnya user baru yang belum punya alamat, atau user yang isi form "Tambah Alamat Baru")
                if (empty($validated['address_id'])) {
                    $isFirstAddress = $customer->addresses()->count() === 0;

                    UserAddress::create([
                        'user_id' => $customer->id,
                        'label' => $isFirstAddress ? 'Alamat Utama' : 'Alamat Lain',
                ...
                // 🔥 GUEST CHECKOUT - JANGAN AUTO-LOGIN!
                // Cek apakah email/phone sudah terdaftar
                $existingCustomer = null;
                
                if (!empty($validated['email'])) {
                    $existingCustomer = User::where('email', $validated['email'])->first();
                }
                
                if (!$existingCustomer && !empty($validated['shipping_phone'])) {
                    $existingCustomer = User::where('phone', $validated['shipping_phone'])->first();
                }

                if ($existingCustomer) {
                    // 🔥 JIKA SUDAH TERDAFTAR, JANGAN AUTO-LOGIN
                    // Kembalikan JSON error agar frontend bisa munculkan login-popup
                    if ($request->ajax() || $request->wantsJson()) {
                        return response()->json([
                            'success' => false,
                            'requires_login' => true,
                            'message' => 'Email atau nomor telepon sudah terdaftar. Silakan login untuk melanjutkan.',
                    ...
                    return redirect()
                        ...
                        ->with('error', 'Email atau nomor telepon sudah terdaftar. Silakan login terlebih dahulu.')
                        ->withInput($request->except(['password', '_token']));
                }

                // 🔥 BUAT USER BARU (GUEST) TANPA AUTO-LOGIN
                ...
                // 🔥 JANGAN AUTO-LOGIN! Customer tetap guest.
                // Auth::guard('customer')->login($customer);  // ❌ HAPUS
                // $request->session()->regenerate();          // ❌ HAPUS
            ...
            // 🔥 BERSIHKAN DATA GUEST CHECKOUT
            session()->forget('guest_checkout_cart');
            session()->forget('guest_checkout_data');
    ...
    public function saveCheckoutData(Request $request)
    {
        try {
            $data = $request->only([
                ...
                'shipping_subdistrict', 'shipping_postal_code',
                'shipping_province_id', 'shipping_city_id',
                'shipping_district_id', 'shipping_subdistrict_id',
                'courier', 'shipping_service', 'shipping_cost',
                'notes', 'voucher_code', 'voucher_discount', 'address_id',
            ]);

            // Simpan ke session
            session()->put('guest_checkout_data', $data);
            session()->put('guest_checkout_saved_at', now());

            // 🔥 Simpan cart ke session terpisah untuk restore setelah login
            session()->put('guest_checkout_cart', session()->get('cart', []));

            \Log::info('Guest checkout data saved', ['data' => $data]);

            return response()->json([
                'success' => true,
                'message' => 'Data checkout berhasil disimpan sementara.',
            ]);
        } catch (\Exception $e) {
            \Log::error('Save checkout data error: ' . $e->getMessage());
    ...
    public function getAvailableVouchers(Request $request)
{
    ...
    public function saveCheckoutAddress(Request $request)
    {
        ...
        // 🔥 MAP form field names ke UserAddress model attributes sebelum validasi
        $request->merge([
            'recipient_name' => $request->input('shipping_name'),
            'recipient_phone' => $request->input('shipping_phone'),
            'address' => $request->input('shipping_address'),
            'city' => $request->input('shipping_city'),
            'district' => $request->input('shipping_district'),
            'subdistrict' => $request->input('shipping_subdistrict'),
            'province' => $request->input('shipping_province'),
            'postal_code' => $request->input('shipping_postal_code') ?: '0',
            'province_id' => $request->input('province_id') ?: $request->input('shipping_province_id'),
            'city_id' => $request->input('city_id') ?: $request->input('shipping_city_id'),
            'district_id' => $request->input('district_id') ?: $request->input('shipping_district_id'),
            'subdistrict_id' => $request->input('subdistrict_id') ?: $request->input('shipping_subdistrict_id'),
        ]);
        ...
        $validated['label'] = $validated['label'] ?? 'Alamat Baru';
