...
@section('page-title', 'Tambah Marketplace')
...
<div class="w-full max-w-4xl mx-auto space-y-6">
        ...
        <div class="min-w-0">
            ...
            <h1 class="text-2xl font-bold flex items-center gap-2.5 mt-2" style="color: var(--text-1)">
                <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl
                    ...
                    <iconify-icon icon="mdi:store-plus-outline" class="text-slate-900 text-2xl"></iconify-icon>
                </span>
                Tambah Marketplace
            </h1>
            <p class="text-sm mt-1.5 ml-12" style="color: var(--text-5)">
                Tambahkan marketplace tempat pelanggan dapat membeli produk.
            </p>
        </div>
    </div>
    ...
    {{-- FORM --}}
    {{-- ============================================ --}}
    <form action="{{ route('admin.marketplaces.store') }}" method="POST" class="space-y-6">
        @csrf

        @include('admin.marketplaces._form', ['marketplace' => null])
