...
class Order extends Model
{
    ...
    public function approveCancellation(int $adminId): bool
    {
        ...
        return \Illuminate\Support\Facades\DB::transaction(function () use ($adminId) {
            ...
            foreach ($this->items as $item) {
                if ($item->variant) {
                    $item->variant->addStock($item->quantity, 'cancellation', "Order {$this->order_number} dibatalkan oleh admin");
                } elseif ($item->product) {
                    $firstVariant = $item->product->variants->first();
                    if ($firstVariant) {
                        $firstVariant->addStock($item->quantity, 'cancellation', "Order {$this->order_number} dibatalkan oleh admin");
                    }
                }
            }
    ...
    {
        // Tidak bisa batalkan jika sudah dikirim, selesai, atau sudah dibatalkan
        $nonCancellable = ['shipped', 'delivered', 'cancelled'];
        
        if (in_array($this->shipping_status, $nonCancellable)) {
            return false;
        }

        // Jika sudah pending cancellation, tidak bisa request lagi
        if ($this->cancellation_status === 'pending') {
            return false;
        }
